...
export class GameInput {
    ...
    constrainViewport(): void {
        ...
        const maxViewportWidth = this.game.state.viewport.width * maxScrollOut;
        const maxViewportHeight = this.game.state.viewport.height * maxScrollOut;

        this.game.state.viewport.x = Math.max(
            -maxViewportWidth,
            Math.min(this.game.state.viewport.x, this.game.state.world.width - this.game.state.viewport.width + maxViewportWidth),
        );

        this.game.state.viewport.y = Math.max(
            -maxViewportHeight,
            Math.min(this.game.state.viewport.y, this.game.state.world.height - this.game.state.viewport.height + maxViewportHeight),
        );
    }
